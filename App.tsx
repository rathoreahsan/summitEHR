...
import HeroImage from './assets/imgs/dashboard-ehr-github.png';
import ProviderImage from './assets/imgs/clinician-first.jpg';
import AdminImage from './assets/imgs/rcm-anlytics.jpg';
import PatientImage from './assets/imgs/patient-portal.jpg';
// --- Hooks & Utilities ---
...
const Hero: React.FC = () => {
  ...
  return (
    <section 
      ...
      className="relative pt-32 pb-20 md:pt-40 md:pb-32 overflow-hidden flex items-center min-h-[90vh]"
    >
      {/* --- RICH ANIMATED BACKGROUND --- */}
      
      {/* 1. Dynamic Base Gradient (Animates colors and position) */}
      <div className="absolute inset-0 bg-gradient-to-br from-purple-100 via-ocean-400 to-summit-300 animate-gradient-slow z-0 pointer-events-none opacity-60" />

      {/* 2. Grid Pattern Overlay */}
      <div className="absolute inset-0 bg-grid-pattern opacity-60 z-0 pointer-events-none"></div>

      {/* 3. Floating Aurora Blobs (Interactive Parallax with Color Cycle) */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 animate-hue-cycle">
        {/* Summit Green/Teal Blob */}
        <div 
          className="absolute -top-[10%] -left-[10%] w-[50%] h-[50%] bg-gradient-to-r from-summit-200 to-teal-100 rounded-full blur-[100px] opacity-60 animate-blob mix-blend-multiply transition-transform duration-100 ease-out"
          style={{ transform: `translate(${mousePos.x * 20}px, ${mousePos.y * 20}px)` }}
        />
        {/* Ocean Blue/Indigo Blob */}
        <div 
          className="absolute top-[20%] -right-[10%] w-[40%] h-[60%] bg-gradient-to-r from-ocean-200 to-indigo-100 rounded-full blur-[100px] opacity-60 animate-blob animation-delay-2000 mix-blend-multiply transition-transform duration-100 ease-out"
          style={{ transform: `translate(${mousePos.x * -30}px, ${mousePos.y * 30}px)` }}
        />
        {/* Soft Purple/Pink Accent Blob */}
        <div 
          className="absolute -bottom-[20%] left-[20%] w-[40%] h-[40%] bg-gradient-to-t from-purple-200 to-pink-100 rounded-full blur-[100px] opacity-50 animate-blob animation-delay-4000 mix-blend-multiply transition-transform duration-100 ease-out"
      ...
      <div className="container mx-auto px-4 md:px-12 relative z-10">
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto mb-16 perspective-1000">
          <div className="reveal reveal-down inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/80 backdrop-blur-sm text-summit-700 text-sm font-bold uppercase tracking-wide mb-6 border border-summit-200 shadow-sm hover:shadow-md transition-all cursor-default transform hover:scale-105">
            <span className="w-2 h-2 rounded-full bg-summit-500 animate-pulse"></span>
            Now featuring Summit Voice AI
          </div>
          
          <h1 className="reveal reveal-up delay-100 text-5xl md:text-7xl font-serif font-bold text-gray-900 mb-6 leading-[1.1] drop-shadow-sm">
            The EHR that listens, <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-summit-600 via-ocean-600 to-summit-600 animate-gradient-slow bg-[length:200%_auto]">learns, and lets you lead.</span>
          </h1>
          ...
          <div className="reveal reveal-up delay-300 flex flex-col sm:flex-row gap-4 justify-center w-full sm:w-auto">
            <button 
              onClick={scrollToDemo}
              className="bg-summit-600 hover:bg-summit-700 text-white font-bold py-4 px-10 rounded-full transition-all shadow-xl shadow-summit-200/50 hover:shadow-summit-300/50 hover:-translate-y-1 flex items-center justify-center gap-2 text-lg active:scale-95"
            >
              Start Free Trial <ArrowRight className="w-5 h-5" />
            </button>
            <button className="bg-white/80 backdrop-blur-sm hover:bg-white text-gray-700 font-bold py-4 px-10 rounded-full border border-gray-200 transition-all hover:border-gray-300 hover:shadow-lg hover:-translate-y-1 flex items-center justify-center gap-2 text-lg active:scale-95">
               See How It Works
            </button>
          </div>
        </div>
        ...
        <div className="relative max-w-6xl mx-auto reveal reveal-zoom delay-300 perspective-1000">
          <div 
            className="relative rounded-2xl overflow-hidden shadow-2xl border border-gray-200/80 bg-white/90 transform transition-transform duration-100 ease-out"
            style={{ 
              transform: `rotateX(${mousePos.y * -4}deg) rotateY(${mousePos.x * 4}deg) scale(1.02)`,
              boxShadow: `${mousePos.x * -20}px ${mousePos.y * 20}px 50px rgba(0,0,0,0.1)`
            }}
          >
            ...
            <img src={HeroImage} alt="EHR Dashboard" className="w-full h-auto mt-12" />
            
            {/* Floating UI Element - Efficiency Score */}
            <div 
              className="absolute top-24 right-8 bg-white/90 backdrop-blur-md p-4 rounded-xl shadow-lg border border-white/40 max-w-xs animate-float hidden md:block transition-transform duration-200 ease-out"
              style={{ transform: `translateZ(40px) translate(${mousePos.x * -30}px, ${mousePos.y * -30}px)` }}
            >
            ...
            {/* Floating UI Element - AI Scribe Active (New) */}
            <div 
              className="absolute bottom-12 left-8 bg-gray-900/90 backdrop-blur-md p-4 rounded-xl shadow-xl border border-gray-700 max-w-xs animate-float hidden md:block transition-transform duration-200 ease-out"
              style={{ 
                animationDelay: '1.5s',
            ...
            >
              <div className="flex items-center gap-3">
                 <div className="w-8 h-8 rounded-full bg-summit-500 flex items-center justify-center text-white animate-pulse"><Microphone className="w-4 h-4"/></div>
                 <div>
                    <div className="text-xs text-summit-300 font-bold uppercase">Summit Voice Active</div>
                    <div className="text-gray-200 text-sm">Generating SOAP Note...</div>
                 </div>
              </div>
            </div>
...
const ComprehensiveFeatures: React.FC = () => {
  ...
  const moreFeatures = [
    { title: 'Telehealth Video', desc: 'HD video visits built directly into the patient chart.', icon: VideoCamera },
    { title: 'Secure Messaging', desc: 'Direct, encrypted text with patients & staff.', icon: Envelope },
    { title: 'Document Mgmt', desc: 'Drag-and-drop faxing, scanning, and filing.', icon: DocumentText },
    { title: 'Analytics', desc: 'Custom reports & real-time MIPS tracking.', icon: ChartBar },
    { title: 'Inventory', desc: 'Track vaccines, meds & DME in real-time.', icon: Cube },
    { title: 'Multi-Location', desc: 'Unified database for all your practice sites.', icon: BuildingOffice },
  ];
    ...
    <section className="py-24 bg-gray-50" id="features">
       <div className="container mx-auto px-4 md:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[minmax(300px,auto)]">
             ...
             <div className="md:col-span-3 bg-gradient-to-br from-summit-600 to-ocean-500 rounded-2xl p-8 md:p-12 shadow-2xl relative overflow-hidden flex flex-col items-center justify-center gap-8 reveal reveal-zoom mt-0">
                 {/* Background effects */}
                 ...
                 <div className="relative z-10 w-full max-w-6xl">
                    <div className="text-center mb-10">
                       <h3 className="text-3xl md:text-4xl font-serif font-bold text-white mb-4">Many More Features to Explore</h3>
                       <p className="text-summit-100 text-lg">Everything you need to run a modern practice, all in one place.</p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
                       {moreFeatures.map((f, i) => (
                          <div key={i} className={`p-4 rounded-xl bg-white/10 border border-white/10 hover:bg-white/20 transition-all duration-300 hover:scale-[1.02] cursor-default group flex items-start gap-4 reveal reveal-up delay-${(i % 3) * 100}`}>
                             <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center shrink-0 group-hover:bg-summit-400 group-hover:text-white transition-colors text-summit-200">
                                <f.icon className="w-6 h-6" />
                             </div>
                             <div>
                                <h4 className="font-bold text-white text-lg mb-1">{f.title}</h4>
                                <p className="text-summit-100/80 text-sm leading-relaxed">{f.desc}</p>
                             </div>
                          </div>
                       ))}
                    </div>
                    ...
                    <button 
                      onClick={scrollToDemo}
                      className="bg-white text-summit-600 font-bold py-4 px-12 rounded-full transition-all shadow-[0_0_20px_rgba(255,255,255,0.3)] hover:shadow-[0_0_30px_rgba(255,255,255,0.5)] hover:-translate-y-1 flex items-center gap-2 mx-auto group text-lg"
                    >
